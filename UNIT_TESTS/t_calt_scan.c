...
int main(void)
{
  ...
  for (ind = 0; ind < sizeof(tests) / sizeof(tests[0]); ++ind) {
    len = caltime_scan(tests[ind].s, &ct);
    printf("[%lu] %s %u\n", ind, tests[ind].s, tests[ind].len);
    test_assert(tests[ind].len == len);
    test_assert(tests[ind].ct.date.year == ct.date.year);
    test_assert(tests[ind].ct.date.month == ct.date.month);
    test_assert(tests[ind].ct.date.day == ct.date.day);
    test_assert(tests[ind].ct.hour == ct.hour);
    test_assert(tests[ind].ct.minute == ct.minute);
    test_assert(tests[ind].ct.second == ct.second);
    test_assert(tests[ind].ct.offset == ct.offset);
  }
